...
export function SoftDeletes<T extends NormalizeConstructor<typeof BaseModel>>(superclass: T) {
  class ModelWithSoftDeletes extends superclass {
     ...
     */
    static withTrashed<Model extends typeof ModelWithSoftDeletes>(this: Model): ModelQueryBuilderContractWithIgnoreDeleted<Model, InstanceType<T>> {
      const query = this.query() as ModelQueryBuilderContractWithIgnoreDeleted<Model, InstanceType<Model>>;
