...
export async function createTables(db: Database) {
  ...
  await db.connection().schema.createTable('users', (table: any) => {
    table.increments()
  ...
  await db.connection().schema.createTable('industries', (table: any) => {
    table.increments()
    table.string('title')
    table.string('text')
  ...
  await db.connection().schema.createTable('books', (table: any) => {
  ...
  await db.connection().schema.createTable('authors', (table: any) => {
    table.increments().primary()
    table.string('name')
    table.timestamps()
    table.timestamp('deleted_at', { useTz: true })
  ...
  await db.connection().schema.createTable('roles', (table: any) => {
    table.increments()
    table.string('title')
    table.timestamps()
  })

  await db.connection().schema.createTable('industry_user', (table: any) => {
    table.increments()
    table.integer('user_id')
    table.integer('industry_id')
  })

  await db.connection().schema.createTable('author_book', (table: any) => {
    table.increments().primary()
  ...
  await db.connection().schema.createTable('posts', (table: any) => {
    table.increments().primary()
    table.string('title')
